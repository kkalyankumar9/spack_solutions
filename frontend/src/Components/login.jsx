...
const Login = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "email") {
      ...
      setInputErrors((prev) => ({ ...prev, email: isValidEmail ? "" : "Enter a valid email address" }));
    } else if (name === "password") {
      const isValidPassword = validatePassword(value);
      setInputErrors((prev) => ({ ...prev, password: isValidPassword ? "" : "Password must contain at least 8 characters, including upper/lowercase, number, and special character" }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response.token) {
        ...
        toast.error(response.msg || "An error occurred during login. Please try again.");
      }
    } catch (error) {
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              <div className="mb-4">
                ...
                <input
                  ...
                  className={`appearance-none rounded-md relative block w-full px-4 py-3 border ${inputErrors.email ? 'border-red-500' : 'border-gray-300'} placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-pink-500 focus:border-pink-500 sm:text-lg`}
                  required
                />
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`appearance-none rounded-md relative block w-full px-4 py-3 border ${inputErrors.password ? 'border-red-500' : 'border-gray-300'} placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-pink-500 focus:border-pink-500 sm:text-lg`}
                  required
                />
